  ...
  return (
    ...
    <form action='/login' method='post' onSubmit={LoginUser} className='flex flex-col items-center justify-center gap-[15px] '>
      ...
      <div className='flex items-center justify-center gap-[20px]'>
        <Link to={"/register"} className='sm:text-[20px] text-[16px] sm:py-[1px] vmd:py-[4px] px-[15px] rounded-md hover:opacity-60 bg-green-500 no-underline'>Register</Link>
      {loader === false ? <button className='sm:text-[20px] text-[16px] sm:py-[1px] vmd:py-[4px] px-[15px] rounded-md hover:opacity-60 bg-blue-500 '>Login</button> :(<Loader></Loader>)}
      </div>
    </form>
